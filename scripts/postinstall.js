...
function install(module, options) {
  ...
  loadPackageJson("./node_modules/meanio/package.json", function(err, data) {
    ...
    getPackage(packageName, function(data) {
      ...
      var cloneUrl = options.git
        ? data.repo
        : "https://git.mean.io/" + data.repo.split(":")[1];
        ...
        function(code) {
          ...
          loadPackageJson(pkgPath + "/package.json", function(err, data) {
            ...
            if (!data.mean) {
              console.log();
              console.log(
                "Warning: The module installed is not a valid MEAN module"
              );
            }
            ...
            shell.exec("npm install .", function(code) {
              ...
              console.log(
                "    Dependencies installed for package " + data.name
              );
    ...
    function(err, npm) {
      fs.readdir(packages, function(err, files) {
        ...
        files.forEach(function(file) {
          ...
          loadPackageJson(path.join(pkgPath, "package.json"), function(
            err,
            data
          ) {
...
cpx.copy(
  ...
  "./packages/custom/icu/public/assets/lib/file-saver/FileSaver.min.js"
);
