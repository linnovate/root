...
const resetPermissions = function (db, callback) {
    ...
    entityNames.forEach(function (entityName) {
        ...
        return promises.push(new Promise(function (resolve, error) {
...
const setPermissions = function (db, callback) {
    ...
    entityNames.forEach(function (entityName) {
        ...
        const collection = db.collection(entityName);
        return promises_set.push(new Promise(function (resolve, error) {
            collection.find().forEach(
                function (elem) {
                    ...
                    const permsPerEntity = elem.watchers.map(watcher => {
                        ...
                        if (elem.creator && elem.creator.valueOf().toString() == watcher) {
                            level = levels[0];
                        }
                        else if (elem.assign && elem.assign.valueOf().toString() == watcher) {
                            level = levels[1];
                        }
                        else {
                            level = levels[2];
