...
describe("Root", function() {
  ...
  describe("Project", function() {
    ...
    it("Set project name", function() {
      ...
      expect(
        element(by.css(".projects tr.active td.name")).getText()
      ).to.eventually.equal(pName);
    });

    it("Set project color", function() {
      element(by.css(".select-box .arrow")).click();
      element
        .all(by.repeater("color in colors"))
        .get(4)
        .click();
      var colorBox = element(by.css(".color-box"));
      expect(colorBox.getAttribute("style")).to.eventually.equal(
    ...
    it("Set task name", function() {
      ...
      expect(
        element(by.css(".projects tr.active td.name")).getText()
      ).to.eventually.equal(tName);
      expect(
        element(by.css(".projects tr.active td.project span")).getText()
      ).to.eventually.equal(pName);
    });
  ...
  describe("Discussion", function() {
    ...
    it("Set assignee", function() {
      ...
      input.sendKeys(protractor.Key.ENTER);
      expect(
        element(by.css("detail-assign .summary-content")).getText()
    ...
    it("Set due date", function() {
      ...
      element(by.css(".ui-datepicker"))
        .element(by.linkText("1"))
        .click();
      browser.waitForAngular();

      element(by.css("#firstStr"))
        .getText()
        .then(function(text) {
          ...
          expect(actualDate.toDateString()).to.equal(
    ...
    it("Create task", function() {
      ...
      element(by.css(".detail-tabs-switcher"))
        .element(by.buttonText("משימות"))
        .click();
