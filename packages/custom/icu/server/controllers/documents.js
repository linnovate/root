...
exports.uploadEmpty = function(req, res, next) {
  var user = req.user.email
    .substring(0, req.user.email.indexOf("@"))
    .toLowerCase();
  var users = [];
        ...
        request(
          ...
          function(error, resp, body) {
            if (error) {
              ...
              var fileName = body.path.substring(
                body.path.lastIndexOf("/") + 1,
                body.path.length
              );
              var fileType = body.path.substring(
                body.path.lastIndexOf(".") + 1,
                body.path.length
              );
              var set = {
              ...
              Document.update({ _id: req.body._id }, { $set: set }, function(
                error,
                result
              ) {
                if (err) {
...
function getFolderIndexMapUpToDate(to) {
  ...
  return Document.aggregate(agg).exec(function(err, indexMapper) {
  ...
  Document.find({ _id: doc._id })
    ...
    .exec(function(err, data) {
      if (err) {
        ...
        var fileName = spPath.substring(
          spPath.lastIndexOf("/") + 1,
          spPath.length
        );
          ...
          function(error, resp, body) {
            if (error) {
              ...
              logger.log(
                ...
                "%s signOnDocx, %s",
                req.user.name,
                "POST:" + "'" + config.SPHelper.uri + "/api/signOnDocx" + "'",
                { error: error.stack }
              );
            } else {
              var set = { spPath: body.path, signBy: signature };
              Document.findOne(
                {
                  _id: doc._id
                },
...
exports.addSerialTitle = function(req, res, next) {
  ...
  if (!doc.creator.username) {
      ...
      .exec(function(err, data) {
        if (err) {
          ...
          var user = req.user.email
            .substring(0, req.user.email.indexOf("@"))
            .toLowerCase();
          var docFolder, docOffice, telOffice, unitOffice, internalTelOffice;
          if (req.body.folder) {
            docFolder = req.body.folder.title
              ? req.body.folder.title
              : "noFolder";
            if (req.body.folder.office) {
              docOffice = req.body.folder.office.title
                ? req.body.folder.office.title
                : "noOffice";
              ...
              if (req.body.folder.office.internalTel) {
                internalTelOffice = req.body.folder.office.internalTel;
              } else {
                internalTelOffice = "noTel";
              }
          ...
          getSerial()
            .then(function(ser) {
              var json = {
                ...
                office: docOffice,
                telOffice: telOffice,
                internalTelOffice: internalTelOffice,
    ...
    var spPath = doc.spPath;
    if (spPath == undefined) {
      logger.log("error", "%s uploadEmpty, %s", req.user.name, " doc.spPath", {
        error: "spPath is undefined"
      });
      res.send("error");
      return;
...
exports.uploadDocumentsFromTemplate = function(req, res, next) {
  ...
  if (template.spPath) {
    var fileName = template.spPath.substring(
      template.spPath.lastIndexOf("/") + 1,
      template.spPath.length
    );
    ...
    var templateUrl = template.spPath.substring(
      template.spPath.indexOf("/ICU"),
      template.spPath.length
    );
                ...
                Document.update(
                  ...
                  function(error, result) {
                    if (error) {
                      logger.log(
                        "error",
                        "%s uploadDocumentsFromTemplate, %s",
                        req.user.name,
                        "Document.update()",
                        { error: error.message }
                      );
    ...
    Document.update({ _id: officeDocument._id }, { $set: set }, function(
      error,
      result
    ) {
      if (error) {
...
exports.getAll = function(req, res, next) {
  ...
  Document.find({
    ...
    .exec(function(err, data) {
      if (err) {
        logger.log("error", "%s getAll, %s", req.user.name, "Document.find", {
          error: err.message
        });
...
exports.getById = function(req, res, next) {
  Document.find(
    ...
    function(err, data) {
      if (err) {
        logger.log("error", "%s getById, %s", req.user.name, "Document.find", {
          error: err.message
        });
        req.locals.error = err;
        res.status(400);
...
exports.getByUserId = function(req, res, next) {
  Document.find(
    {
      $or: [
        { watchers: { $elemMatch: { $eq: req.params.id } } },
        { asign: req.params.id }
      ]
    },
    function(err, data) {
...
exports.upload = function(req, res, next) {
  ...
  busboy.on("file", function(fieldname, file, filename) {
    ...
    var saveTo = path.join(
      ...
      d,
      new Date().getTime() + "-" + path.basename(filename)
    );
    var hostFileLocation =
      config.host + ":" + port + saveTo.substring(saveTo.indexOf("/files"));
    var fileType = path
      .extname(filename)
      .substr(1)
      .toLowerCase();
    mkdirp(path.join(config.attachmentDir, d), function() {
      file.pipe(fs.createWriteStream(saveTo)).on("close", function(err) {
        if (err) {
          logger.log("error", "%s upload, %s", req.user.name, "mkdirp", {
            error: err.message
          });
        }
  ...
  busboy.on("finish", function() {
    var user = req.user.email.substring(0, req.user.email.indexOf("@"));
    var path = req.locals.data.body.path.substring(
      req.locals.data.body.path.indexOf("/files"),
      req.locals.data.body.path.length
    );
    var fileName = path.substring(path.lastIndexOf("/") + 1, path.length);
    req.locals.data.body.path =
      config.SPHelper.SPSiteUrl +
      "/" +
      config.SPHelper.libraryName +
      "/" +
      user +
      "/" +
      req.locals.data.body.name;
                  ...
                  function(error, resp, body) {
                    if (error || !body) {
                      ...
                      var doc = {
                        ...
                        documentType: fileName.substring(
                          fileName.indexOf(".") + 1,
                          fileName.length
                        )
                      };
                  ...
                  request(
                    ...
                    function(error, resp, body) {
                      if (error) {
                        ...
                        var path = req.locals.data.body.originalPath;
                        var fileType = path.substring(
                          path.lastIndexOf(".") + 1,
                          path.length
                        );
                        var set = {
                          path: req.locals.data.body.originalPath,
                          title: req.locals.data.body.name,
                          documentType: fileType
                        };
                        Document.findOne({ _id: documentId }, function(
                          err,
                          doc
                        ) {
...
function addToParent(parentType, parentId, childId){
    ...
    return Model.findOne({_id: parentId})
        .then( doc => {
            if(!_.includes(doc.officeDocuments, childId))doc.officeDocuments.push(childId)
            doc.save();
        })
}
  ...
  Document.find({ _id: req.params.id }, function(err, file) {
    if (err) {
      ...
      if (spPath) {
        ...
        var folderName = spPath.substring(
          spPath2.lastIndexOf("/") + 1,
          spPath2.length
        );
        var spPath2 = spPath2.substring(0, spPath2.lastIndexOf("/"));
        var libraryName = spPath2.substring(
          spPath2.lastIndexOf("/") + 1,
          spPath2.length
        );
        var user = req.user.email.substring(0, req.user.email.indexOf("@"));
...
exports.update2 = function(req, res, next) {
  ...
  getDocuments(entity, id)
    ...
    .catch(function(err) {
      logger.log("error", "%s update2, %s", req.user.name, " getUsers", {
        error: err.message
      });
    });
};
...
exports.update = function(req, res, next) {
  ...
  if (req.body.name) {
    if (req.body.name == "watchers" || req.body.watchers) {
      var watchers =
        req.body.name == "watchers"
          ? req.body.newVal
          : req.body.watchers.map(function(w) {
              return w._id;
            });
      Document.findOne({ _id: req.params.id }, function(err, doc) {
        ...
        Folder.findOne({ _id: req.body.newVal }).exec(function(err, folderObj) {
          if (err) {
            logger.log(
              "error",
              "%s create, %s",
            ...
            if (spPath) {
              ...
              getUsers(creator)
                .then(function() {
                  getUsers(watchersReq)
                    .then(function() {
                      getUsers(oldWatchersReq)
                        .then(function() {
                          var users = [],
                            zero = [];
      ...
      Document.findOne({ _id: req.params.id }, function(err, doc) {
        ...
        if (spPath) {
          ...
          getUsers(assignReq)
            ...
            .catch(function(err) {
              logger.log("error", "%s update, %s", req.user.name, " getUsers", {
                error: err.message
              });
            });
        }
...
exports.signNew = function(req, res, next) {
  ...
  query
    ...
    .exec(function(err, entity) {
      if (err) {
        logger.log("error", "%s signNew, %s", req.user.name, " query", {
          error: error.message
        });
...
exports.receiveDocument = function(req, res, next) {
  ...
  var id = req.params.id;
  Document.update(
    { _id: officeDocument.ref },
    { $push: { readBy: { date: Date.now(), user: req.user._id } } },
    function(error, result) {
      if (error) {
        //TBD
        logger.log(
  ...
  Document.update({ _id: id }, { $set: { viewed: true } }, function(
    error,
    result
  ) {
    if (error) {
...
exports.distributedDocument = function(req, res, next) {
  officeDocument = req.body.officeDocument;
  var id = req.params.id;
  Document.update({ _id: id }, { $set: { status: "viewed" } }, function(
    error,
    result
  ) {
    if (error) {
...
exports.sendDocument = function(req, res, next) {
  ...
  var creators = officeDocument.creator
    ? [officeDocument.creator.username.toLowerCase()]
    : [];
  sendingForm["doneBy"] = sendingForm["doneBy"] ? sendingForm["doneBy"] : [];
  sendingForm["forNotice"] = sendingForm["forNotice"]
    ? sendingForm["forNotice"]
    : [];
  sendingForm["sendingAs"] = sendingForm["sendingAs"]
    ? sendingForm["sendingAs"]
    : null;
  sendingForm["ref"] = req.body.officeDocument._id;
  sendingForm["sender"] = req.user._id;
  var watchers = _.union(
    sendingForm["doneBy"],
    _.union(
      sendingForm["forNotice"],
      _.union([assign], [sendingForm["sendingAs"]])
    )
  );
    ...
    if (watcher != sendingForm.sender) {
      ...
      doc2.save(function(error, result) {
        if (error || !result) {
          ...
          Attachment.find({ entityId: officeDocument._id }).then(function(
            attachments
          ) {
  ...
  Document.findOne(
    ...
    function(err, prevDoc) {
      ...
      sentTo = [];
      watchers.forEach(function(watcher) {
        watcher != sendingForm.sender
          ? sentTo.push({ date: Date.now(), user: watcher })
          : "";
      });
      if (sentTo.length) {
...
var copyFile = function(file, dir2) {
  ...
  file =
    "/home/sraya/Desktop/ICU_25.12.17/root/files/2018/01/22/1516632480971-Combined.pdf";
  ...
  fs.stat(file, function(err, stat) {
    ...
    readStream.pipe(
      fs.createWriteStream(
        "/home/sraya/Desktop/ICU_25.12.17/root/files/2018/01/22/yyyy.pdf"
      )
    );
  });
};
