...
function update(req, res, next) {
  ...
  let { ids, update, remove } = req.body;

  Model.find({ _id: { $in: ids } })
    .then(docs => {
      if (!docs.length) throw new httpError(404);
      if (!checkBoldedPermissions(docs, req.user))
    ...
    .then(function(docs) {
      docs = docs.map(doc => {
        if (update) {
          let { status, due, assign, watchers, tags, permissions } = update;
          ...
          if (watchers && watchers.length) {
            doc.watchers = _.union(
              doc.watchers.map(v => v.toString()),
              watchers
            );
          }
          if (tags && tags.length) {
        ...
        if (remove) {
          let { watchers, tags } = remove;
          if (tags && tags.length) {
            doc.tags = _.difference(doc.tags, tags);
          }
          if (watchers && watchers.length) {
            doc.watchers = _.difference(
              doc.watchers.map(v => v.toString()),
              watchers
            );
            watchers.forEach(watcher => {
              let permIndex = doc.permissions.findIndex(
                p => p.id.toString() === watcher
              );
...
function recycle(req, res, next) {
  ...
  Model.find(findArrayIds)
    .then(function(docs) {
      ...
      return Model.update(findArrayIds, recycledSet, { multi: true });
    })
    .then(function(results) {
      return Model.find(findArrayIds).populate("creator");
    })
    .then(docs => {
      if (!docs.length) throw new httpError(404);
...
function checkIncludesDueFields({
  startDate,
  endDate,
  startTime,
  endTime,
