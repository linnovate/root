...
exports.updateContent = function(user, oldDoc, newDoc) {
  ...
  let newPerms =
    newDoc && Array.isArray(newDoc["permissions"])
      ? newDoc["permissions"].slice()
      : [];
  let oldPerms =
    oldDoc && Array.isArray(oldDoc["permissions"])
      ? oldDoc["permissions"].slice()
      : [];
...
exports.createContent = function(user, oldDoc, newDoc) {
  ...
  if (newDoc.updateField === "comment" || newDoc.updateField === "attachment") {
    ...
    Model.findOne({ _id: newDoc.entity }, function(err, doc) {
      ...
      // console.log(doc) ;
      let newPerms =
        doc && Array.isArray(doc["permissions"])
          ? doc["permissions"].slice()
          : [];
      if (!exports.allowUpdateUpdates(user, newPerms, {})) {
        console.log("updates - not allowed");
        deffered.reject(
          exports.permError.denied + ":" + exports.permError.allowUpdateUpdates
        );
        return deffered.promise;
      }
  ...
  if (
    newDoc.updateField == "assign" &&
    newDoc.entityType == "officeDocuments"
  ) {
    ...
    Model.findOne({ _id: newDoc.entity }, function(err, doc) {
      ...
      let exist = exports.searchIdIndex(
        String(assigneeAdded),
        doc["permissions"]
      );
  ...
  if (
    newDoc.updateField == "updateWatcher" &&
    newDoc.entityType == "officeDocuments"
  ) {
    ...
    Model.findOne({ _id: newDoc.entity }, function(err, doc) {
      ...
      let permLevel =
        String(watcherAdded) == String(newDoc.assign) ? "commenter" : "viewer";
  ...
  if (
    newDoc.updateField == "removeWatcher" &&
    newDoc.entityType == "officeDocuments"
  ) {
  ...
  Model.findOne({ _id: newDoc.entity }, function(err, doc) {
    ...
    if (!exports.allowUpdateContent(user, newPerms, {})) {
      ...
      deffered.reject(
        exports.permError.denied + ":" + exports.permError.allowCreateContent
      );
      return deffered.promise;
    } else {
...
exports.updatePermsArray = function(user, oldDoc, newDoc) {
  ...
  // console.log(JSON.stringify(newDoc)) ;
  let oldWatchers =
    oldDoc.watchers instanceof Array
      ? oldDoc.watchers.map(function(item) {
  ...
  let newWatchers =
    oldDoc.watchers instanceof Array
      ? newDoc.watchers.map(function(item) {
          return String(item);
        })
      : [];
  ...
  if (watcherAdded.length > 0) {
    ...
    watcherAdded.forEach(function(watcher) {
      let permLevel =
        String(watcher) == String(newDoc.assign) ? "commenter" : "viewer";
  ...
  return new Promise((resolve, reject) => {
    ...
    Model.findOne({ _id: id })
      ...
      .then(doc => {
        ...
        let permissions = doc.permissions.filter(
          permission => String(permission.id) === String(user._id)
        )[0];
        if (_.includes(permsAllowed, permissions.level)) allowed = true;
        if (permissions.level === "commenter")
          allowed = String(file.creator) === String(user._id);
