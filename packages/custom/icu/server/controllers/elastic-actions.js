...
function insertOffsetToIndexKue(schema) {
  ...
  Collection.count(function(err, count) {
    ...
    for (offset = 0; offset < countItem; ) {
      queue
        .create("index", { schema: schema, offset: offset })
        .priority("high")
...
function indexJob(data, done) {
  ...
  Collection.find()
    ...
    .toArray(function(err, nodes) {
      if (!err) {
        indexCollectionElastic(collectionName, nodes, elasticObj).then(
          function(res) {
...
exports.indexCategories = function(data, type, elastic, fieldName) {
  ...
  if (type === "pc_s") {
    ...
    obj.vid = data.vid;
    obj.featureParent =
      data.field_features && data.field_features.length
        ? data.field_features[0].tid
        : "";
    obj.field_ps_field_type = data.field_ps_field_type
      ? data.field_ps_field_type
      : "";
    obj.parentFeatureText = data.parentFeatureText
      ? data.parentFeatureText
      : "";
  }
...
exports.updateTaxonomyCounter = function(data, elastic) {
  ...
  var queryField = {
    field_ps_categories: 1,
    field_categories_features: 1,
    status: 1,
    old_status: 1
  };
  ...
  Collection.findOne(query, queryField, function(err, doc) {
    ...
    if (doc.status !== doc.old_status) {
      ...
      CollectionTax.update(
        { tid: { $in: tids } },
        { $inc: { counter: flag } },
        { multi: true },
        function(err, status) {
          CollectionTax.find({ tid: { $in: tids } }).toArray(function(
            err,
            tidsObj
          ) {
    ...
    .toArray(function(err, users) {
      if (!err && users.length) {
        users.forEach(function(user) {
          ...
          if (!user.publishFields) user.publishFields = {};

          user.publishFields = {
            firstName:
              user.publishFields.firstName !== undefined
                ? user.publishFields.firstName
                : true,
            lastname:
              user.publishFields.lastname !== undefined
                ? user.publishFields.lastname
                : true,
            companyName:
              user.publishFields.companyName !== undefined
                ? user.publishFields.companyName
                : true,
            companyWebsite:
              user.publishFields.companyWebsite !== undefined
                ? user.publishFields.companyWebsite
                : true,
            companyLogo:
              user.publishFields.companyLogo !== undefined
                ? user.publishFields.companyLogo
                : true,
            mail:
              user.publishFields.mail !== undefined
                ? user.publishFields.mail
                : false,
            field_address_text:
              user.publishFields.field_address_text !== undefined
                ? user.publishFields.field_address_text
                : !!user.commercial,
            field_geo_points:
              user.publishFields.field_geo_points !== undefined
                ? user.publishFields.field_geo_points
                : !!user.commercial,
            allAddress:
              user.publishFields.allAddress !== undefined
                ? user.publishFields.allAddress
                : !!user.commercial,
            field_address_guidlines:
              user.publishFields.field_address_guidlines !== undefined
                ? user.publishFields.field_address_guidlines
                : !!user.commercial,
            field_groups:
              user.publishFields.field_groups !== undefined
                ? user.publishFields.field_groups
                : true,
            field_organizations:
              user.publishFields.field_organizations !== undefined
                ? user.publishFields.field_organizations
                : true,
            picture:
              user.publishFields.picture !== undefined
                ? user.publishFields.picture
                : true,
            about:
              user.publishFields.about !== undefined
                ? user.publishFields.about
                : true,
            main_business:
              user.publishFields.main_business !== undefined
                ? user.publishFields.main_business
                : true,
            google:
              user.publishFields.google !== undefined
                ? user.publishFields.google
                : true,
            facebook:
              user.publishFields.facebook !== undefined
                ? user.publishFields.facebook
                : true
          };
          ...
          user.phone.forEach(function(phone) {
            if (phone.publishPhone === undefined) phone.publishPhone = true;
          });
...
exports.setUserPublishedFields = function(req, res) {
  ...
  CollectionUser.count(function(err, count) {
    ...
    for (offset = 0; offset < countItem; ) {
      queue
        .create("userPublishedFields", { offset: offset })
        .priority("high")
        .removeOnComplete(true)
        .save();
      offset = limit + offset;
