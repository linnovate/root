...
exports.excel = function(req, res, next) {
  ...
  //if user selected tasks from project or discussion the excel generated will contain only tasks from that entity
  if (entity && entityId)
    tasks = _.filter(tasks, t => {
      if (t[entity]) {
        const predicate = t[entity] && t[entity]._id.toString() === entityId;
        return predicate;
      } else return false;
    });
