...
exports.all = function(req, res) {
  ...
  mean.elasticsearch.search(
    ...
    function(err, response) {
      if (err) next(new Error("Failed to found documents"));
      else
        res.send(
          response.hits.hits.map(function(item) {
            return item._source;
          })
        );
    }
...
exports.update = function(req, res, next) {
  ...
  CommentModel.findById(req.params.id, function(err, comment) {
    if (err) return next(err);
    if (!comment)
      return next(new Error("Cannot find comment with id: " + req.params.id));
    comment = _.extend(comment, req.body);
...
exports.destroy = function(req, res, next) {
  ...
  CommentModel.findById(req.params.id, function(err, comment) {
    ...
    if (!comment)
      return next(
        new Error("Cannot find comment with id: " + req.params.id, next)
      );
    comment.remove(
      {
        user: req.user,
...
exports.readHistory = function(req, res, next) {
  ...
  } else next(new Error(req.params.id + " is not a mongoose ObjectId"));
};
