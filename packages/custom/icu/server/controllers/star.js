...
function filterByStarred(req, res, next) {
  ...
  if (!req.locals.result || req.locals.result.length === 0 || !starred)
    return next();
  req.params.entity = "tasks";

  findStarred(req).then(function(starred) {
    if (starred)
      req.locals.result = starred.filter(starred =>
        req.locals.result.find(
          entity => entity._id.toString() === starred._id.toString()
        )
      );
    next();
  });
...
function findStarred(req) {
  ...
  const entityName =
      req.params.type === "byAssign"
        ? {
            name: req.params.entity || req.locals.data.entityName,
            assign: true
          }
