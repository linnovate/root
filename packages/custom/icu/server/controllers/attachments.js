...
exports.upload = function(req, res, next) {
  ...
  busboy.on("file", function(fieldname, file, filename) {
    ...
    var hostFileLocation =
      config.host + portStr + saveTo.substring(saveTo.indexOf("/files"));
    var fileType = path
      .extname(filename)
      .substr(1)
      .toLowerCase();
...
exports.deleteFile = function(req, res) {
  Attachment.find({ _id: req.params.id }, function(err, file) {
    if (err || !file) {
      logger.log(
        "error",
        "%s deleteFile, %s",
      ...
      return permissions
        .allowRemoveAttachments(req.user, file[0], req.query)
        .then(
          allow => {
                ...
                Attachment.remove({ _id: req.params.id }, function(err) {
                  if (err) {
                    ...
                    if (!err && c <= 1) {
                      ...
                      pathPDF =
                        pathPDF.substring(0, pathPDF.lastIndexOf(".")) + ".pdf";
                      res.status(200).send({ status: "OK" });
                      ftp
                        .archiveFileFromFtp(pathFile)
                        .then(function() {
                          if (!flag) {
                            ftp
                              .deleteFileFromFtp(pathPDF)
                              .then(function() {
                                res.send("ok");
                              })
                              .catch(function(err) {
...
exports.getByPath = function(req, res, next) {
  ...
  query.findOne(conditions).exec(function(err, attachment) {
    if (err || !attachment) {
      ...
      query.findOne(conditions).exec(function(err, attachment) {
        if (err || !attachment) {
          var query = require("mongoose").model("TemplateDoc");
          query.findOne(conditions).exec(function(err, attachment) {
            if (err || !attachment) {
              req.locals.error = {
                status: 404,
...
exports.getAttachmentsByIds = function(req, res, next) {
  ...
  Attachment.find({ issueId: { $in: activitiesIds } }).exec(function(
    err,
    documents
  ) {
    if (err) {
