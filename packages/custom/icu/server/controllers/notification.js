...
var notifications = require("../root-notifications")({
  //CHANGE TO 'root-notifications' IN ORDER TO TAKE FROM node_modules
...
exports.updateRoom = async function(req, res, next) {
  ...
  toAdd.map(async username => {
    try {
      ...
      logger.log("error", "error adding user to RocketChat Group", {
        error: err
      });
    }
  });
  ...
  toKick.map(async username => {
    try {
      ...
      logger.log("error", "error kicking user from RocketChat Group", {
        error: err
      });
    }
  });
...
exports.updateTaskNotification = function(req, res, next) {
  ...
  for (var i in data) {
    if (hiSettings.taskNotify[i] && hiSettings.taskNotify[i].chat) {
      ...
      if (
        i == "due" &&
        (typeof data[i] !== "undefined" || typeof oldData[i] !== "undefined")
      ) {
...
var bulidMassage = function(context) {
  ...
  if (context.action == "added") context.type = "new " + context.type;
  var msg = _.capitalize(context.type);
  if (context.name) msg += ' "' + context.name + '"';
  if (context.sub !== "sub-task") msg = msgWithUrl(msg, context.url);
  ...
  if (context.proj && context.proj_url) {
    if (context.action == "updated") msg += " from project ";
    else if (context.action == "added") msg += " to project ";
  ...
  if (context.issue)
    msg +=
      " to " +
      msgWithUrl(
        context.issue + ' "' + context.issueName + '"',
        context.location
      );
  if (context.user) msg += " by " + _.capitalize(context.user);
  ...
  if (context.sub == "sub-task" && context.action == "added") {
    ...
    if (context.parent) msg += msgWithUrl(context.parent, context.url);
  }
  return msg;
};
