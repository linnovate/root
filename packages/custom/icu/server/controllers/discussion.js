...
exports.schedule = function(req, res, next) {
  ...
  if (
    !(
      (discussion.startDate && discussion.allDay) ||
...
exports.summary = function(req, res, next) {
  ...
  query
    ...
    .then(function(tasks) {
      ...
      Discussion.populate(discussion, options, function(err, doc) {
        if (err || !doc) return next();
        discussion.title = striptags(discussion.title);
        mailService
          .send("discussionSummary", {
            discussion: discussion,
            projects: projects,
            additionalTasks: additionalTasks
          })
          .then(function() {
            var taskIds = _.reduce(
              tasks,
              function(memo, task) {
                var containsAgenda = !_.any(task.discussions, function(d) {
                  return (
                    d.id !== discussion.id &&
                    (d.status === "new" || d.status === "scheduled")
                  );
                });

                var shouldRemoveTag =
                  task.tags.indexOf("Agenda") !== -1 && containsAgenda;
            ...
            var taskIds = _.reduce(
              ...
              function(memo, task) {
                var containsAgenda = !_.any(task.discussions, function(d) {
                  return (
                    d.id !== discussion.id &&
                    (d.status === "new" || d.status === "canceled")
                  );
                });
    ...
    query.exec(function(err, discussions) {
      if (err) {
        ...
        if (pagination.sort == "custom") {
          ...
          Order.find(
            { name: "Discussion", project: discussions[0].project },
            function(err, data) {
              data.forEach(function(element) {
                for (var index = 0; index < discussionTemp.length; index++) {
                  if (
                    JSON.stringify(discussionTemp[index]._id) ===
