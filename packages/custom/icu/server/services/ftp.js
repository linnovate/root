...
function uploadToFTP(path, onlyPDF, pdfURL) {
  return new Promise(function(fulfill, reject) {
    ...
    var pdf =
      ...
      ".pdf";
    var pdfFTPPath = ftpPath.substring(
      ftpPath.indexOf("/"),
      ftpPath.lastIndexOf(".")
    );
    ...
    if (onlyPDF) {
      uploadClient.on("ready", function() {
        ...
        path = decode(path);
        exec("sudo lowriter --headless --convert-to pdf " + path, function(
          err,
          stout,
          sterr
        ) {
          if (err) {
            ...
            var pdfPath = stout.substring(
              stout.indexOf("->") + 3,
              stout.lastIndexOf(".pdf") + 4
            );
              ...
              uploadClient.mkdir(pdfDir, true, function() {
                uploadClient.put(pdf, pdfURL, function(err) {
                  ...
                  if (err) {
                    ...
                    pdfFilesPath =
                      "./files/00/00/00/" +
                      pdf.substring(pdf.lastIndexOf("/") + 1, pdf.length);
                    decodedPdf = decode(pdf);
                    decodedFilesPath = decode(pdfFilesPath);
                    exec(
                      "sudo mv " + decodedPdf + " " + decodedFilesPath,
                      function(err, stout, sterr) {
                        ...
                        if (err) {
                          ...
                          fulfill(
                            pdfFilesPath.substring(1, pdfFilesPath.length)
                          );
                        }
                      }
                    );
    ...
    getClient.on("ready", function() {
      getClient.get(fileUrl, function(err, stream) {
        if (err || !stream) {
          ...
          stream.pipe(fs.createWriteStream(filePath));
          stream.on("finish", function() {
            uploadToFTP(filePath, true, pdfURL)
...
exports.getFileFromFtp = function(req, res, next) {
  ...
  if (url.indexOf("&docType=") != -1) {
    ...
    docType = req.params.url.substring(
      req.params.url.lastIndexOf("=") + 1,
      req.params.url.length
    );
  }
  url = url.replace(/%2f/g, "/");

  getFromFTPByURL(url, downloadAndDelete)
    .then(function(data) {
      res.end(data);
    })
    .catch(function() {
      if (url.startsWith("preview")) {
        var realFilePath = url
          .replace("preview", "files")
          .replace(".pdf", "." + docType);

        uploadPDFOfFile(realFilePath, url)
          .then(function(result) {
            var json = {
              pathForView: result
            };
