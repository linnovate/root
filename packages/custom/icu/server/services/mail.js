...
var EmailTemplate = require("../../../mail-templates/node_modules/email-templates")
  .EmailTemplate;
...
function writeDocxToFile(docx, discussionId) {
  return new Promise(function(fulfill, reject) {
    ...
    out.on("error", function(err) {
      logger.log("error", "writeDocxToFile, %s", " fs.createWriteStream", {
        error: err.message
      });
      reject(err);
    });
    docx.generate(out, {
      finalize: function(written) {
        exec("lowriter --headless --convert-to pdf " + path, function(
          err,
          stout,
          sterr
        ) {
          if (err) {
            logger.log("error", "writeDocxToFile, %s", " convert-to pdf", {
              error: err.message
            });
            reject(err);
            return;
            ...
            exec(
              ...
              function(err, stout, sterr) {
                if (err) {
                  logger.log("error", "writeDocxToFile, %s", " save pdf file", {
                    error: err.message
                  });
                  reject(err);
                } else {
      ...
      error: function(err) {
        logger.log("error", "writeDocxToFile, %s", " docx.generate", {
          error: err.message
        });
        reject(err);
      }
...
function addTasks(docx, tasks, discussionId, tasksNum) {
  return new Promise(function(fulfill, reject) {
    if (!tasks || tasks == null || tasks.length == 0) {
      var assigns = [];
      addTasksToDocx(docx, tasks, assigns, discussionId, tasksNum)
        .then(function(result) {
          if (result == "success") {
            fulfill("success");
          } else {
            logger.log("error", "addTasks, %s", " addTasksToDocx", {
              error: "Unknown Error"
            });
      ...
      (function func() {
        User.findOne({
          ...
          if (err) {
            logger.log("error", "addTasks, %s", " User.findOne", {
              error: err.message
            });
            reject(err);
            return;
...
function addTasksToDocx(docx, tasks, assigns, discussionId, tasksNum) {
  return new Promise(function(fulfill, reject) {
    ...
    if (tasks == null || tasks == undefined || tasks.length == 0) {
      writeDocxToFile(docx, discussionId).then(function(result) {
        if (result == "success") {
          ...
          logger.log("error", "addTasksToDocx, %s", " writeDocxToFile", {
            error: "Unknown Error"
          });
    ...
    for (var i = 0; i < tasksNum; i++) {
      ...
      var flag =
        currentTask.title.charAt(0) >= "א" &&
        currentTask.title.charAt(0) <= "ת";
      ...
      pObj.addText(striptags(currentTask.title), {
        font_size: 14,
        font_face: "David",
        bold: true,
        underline: true
      ...
      if (currentTask.description) {
        ...
        var desc = currentTask.description
          ? currentTask.description.substring(
              3,
              currentTask.description.length - 4
            )
          : "";
        var pObj = docx.createP({ align: "right" });
      ...
      if (currentTask.watchers && currentTask.watchers.length > 0) {
        ...
        for (var j = 0; j < currentTask.watchers.length; j++) {
          ...
          var watcher =
            currentTask.watchers[j].name +
            " " +
            currentTask.watchers[j].lastname;
      ...
      if (currentTask.assign) {
        ...
        if (flag) {
          ...
          pObj.addText(assign + " " + currentTask.assign.lastname, {
            font_size: 14,
            font_face: "David"
          });
        }
      ...
      if (currentTask.due) {
        ...
        var timeString =
          due != null
            ? (due.getHours().toString().length == 1
                ? "0" + due.getMinutes().toString()
                : due.getMinutes().toString()) +
              ":" +
              (due.getMinutes().toString().length == 1
                ? "0" + due.getMinutes().toString()
                : due.getMinutes().toString())
            : "";
        pObj.addText("תג״ב: ", {
    ...
    writeDocxToFile(docx, discussionId)
      .then(function(result) {
        if (result == "success") {
          fulfill("success");
        } else {
...
function createPDF(discussion, tasks) {
  return new Promise(function(fulfill, reject) {
    ...
    docx.on("error", function(err) {
      logger.log("error", "createPDF, %s", " docx (officegen)", {
        error: err.message
      });
    });
    ...
    if (discussion.title) {
      //Discussion title
      var flag =
        discussion.title.charAt(0) >= "א" && discussion.title.charAt(0) <= "ת";
      if (!flag) {
        header.addText("דיון: ", {
          font_size: 16,
          font_face: "David",
        ...
        header.addText(striptags(discussion.title), {
          font_size: 16,
          font_face: "David",
          underline: true
        });
    ...
    if (discussion.assign) {
      ...
      var pObj = docx.createP({ align: "right" });
      var flag =
        discussion.assign.name.charAt(0) >= "א" &&
        discussion.assign.name.charAt(0) <= "ת";

      if (flag) {
        pObj.addText("אחראי הדיון: ", {
          font_size: 14,
          font_face: "David",
          bold: true,
          underline: true
        ...
        pObj.addText(
          discussion.assign.name + " " + discussion.assign.lastname,
          { font_size: 14, font_face: "David" }
        );
      }
      if (!flag) {
        if (discussion.assign.lastname) {
          pObj.addText(discussion.assign.lastname + " ", {
            font_size: 14,
            font_face: "David"
          });
        }
    ...
    if (discussion.watchers && discussion.watchers.length > 0) {
      ...
      for (var i = 0; i < discussion.watchers.length; i++) {
        pObj.addText(
          discussion.watchers[i].name + " " + discussion.watchers[i].lastname,
          { font_size: 14, font_face: "David" }
        );
    ...
    if (discussion.location) {
      ...
      var pObj = docx.createP({ align: "right" });
      var flag =
        discussion.location.charAt(0) >= "א" &&
        discussion.location.charAt(0) <= "ת";
      if (flag) {
        pObj.addText("מיקום: ", {
          font_size: 14,
          font_face: "David",
    ...
    addTasks(docx, tasks2, discussion._id, tasks.length)
      .then(function(result) {
        if (result == "success") {
          fulfill("success");
        } else {
          logger.log("error", "createPDF, %s", " addTasks", {
            error: "Unknown Error"
          });
          reject("error");
        }
...
exports.send = function(type, data) {
  ...
  exec("pwd | tr -d '\n'", function(error, stdout, stderr) {
    ...
    createPDF(data.discussion, data.additionalTasks)
      .then(function(result) {
...
exports.sendMyTasksOfTodaySummary = function(type, data) {
  ...
  exec("pwd | tr -d '\n'", function(error, stdout, stderr) {
    if (error) {
      logger.log("error", "sendMyTasksOfTodaySummary, %s", " pwd", {
        error: error.message
      });
    }
    rootPath = stdout;
    render(type, data)
      ...
      .catch(function(err) {
        logger.log("error", "sendMyTasksOfTodaySummary, %s", " render", {
          error: err
        });
      });
  });
...
exports.sendGivenTasksOfTodaySummary = function(type, data) {
  if (type === "comment_email") {
    return;
  }
  var rootPath = "";
  ...
  builder.throwError = false;
  builder.ignoreTZIDMismatch = true;
  builder.calname = "This Given Today tasks";
  builder.timezone = "asia/istanbul";
  builder.tzid = "asia/istanbul";
  builder.method = calMethod;
  ...
  exec("pwd | tr -d '\n'", function(error, stdout, stderr) {
    if (error) {
      logger.log("error", "sendGivenTasksOfTodaySummary, %s", " pwd", {
        error: error.message
      });
    }
    rootPath = stdout;
    render(type, data)
      .then(function(results) {
        var promises = emails.map(function(recipient) {
          var mailOptions = {
            to: recipient,
            from: config.emailFrom,
            subject: "This Given Today tasks",
            html: results.html,
            text: results.text,
            forceEmbeddedImages: true
          };
      ...
      .catch(function(err) {
        logger.log("error", "sendGivenTasksOfTodaySummary, %s", " render", {
          error: err
        });
      });
  });
...
exports.sendMyTasksOfNextWeekSummary = function(type, data) {
  ...
  exec("pwd | tr -d '\n'", function(error, stdout, stderr) {
    if (error) {
      logger.log("error", "sendMyTasksOfNextWeekSummary, %s", " pwd", {
        error: error.message
      });
    }
    rootPath = stdout;
    render(type, data)
      ...
      .catch(function(err) {
        logger.log("error", "sendMyTasksOfNextWeekSummary, %s", " render", {
          error: err.message
        });
      });
  });
...
exports.sendGivenTasksOfNextWeekSummary = function(type, data) {
  ...
  exec("pwd | tr -d '\n'", function(error, stdout, stderr) {
    if (error) {
      logger.log("error", "sendGivenTasksOfNextWeekSummary, %s", " render", {
        error: error.message
      });
    }
    rootPath = stdout;
