...
module.exports = function(entityName, options) {
  ...
  if (findByUser.indexOf(entityName) > -1) var currentUser = true;
  ...
  function all(pagination, user, acl) {
    ...
    if (currentUser) {
      query = require("mongoose").model(entityNameMap[entityName].name);
      countQuery = require("mongoose")
        .model(entityNameMap[entityName].name)
    ...
    if (pagination && pagination.type) {
      if (pagination.type === "page") {
        query = query
          ...
          .catch(err => {
            console.log(err);
            console.log("really big error type page");
          });
        mergedPromise = q
          .all([query, countQuery])
      ...
      console.log("im here");
      // console.log(query);
      query = query.find(options.conditions).populate(options.includes || "");
      // .then(res => {
      //   console.log("res something something");
      //   console.log(res);
      //   deffered.resolve(res);
      // });
      //!need to check if it affect performence
      // query.hint({
      //   _id: 1
      // });
  ...
  function create(entity, user, acl) {
    ...
    if (!allowed1) {
      ...
      return throwError(
        permissions.permError.denied +
          ":" +
          permissions.permError.allowUpdateWatcher
      );
    }
    ...
    allowed2
      ...
      .then(function(value) {
        ...
        return permissions.cloneParentPermission(
          entity.parent,
          entityNameMap[model.collection.collectionName].name.toLowerCase()
        );
      })
      .then(function(clonedPerms) {
        ...
        entity.permissions = clonedPerms.permissions || [
          { id: String(user.user._id), level: "editor" }
        ];
        deffered.resolve(
          new Model(entity).save(user).then(function(e) {
  ...
  function update(oldE, newE, user, acl) {
    ...
    if (!allowed2) {
      return throwError(
        permissions.permError.denied +
          ":" +
          permissions.permError.allowUpdateContent
      );
    }
    ...
    // circlesAcl.sign("mongoose", oldE.sources, oldE.circles, acl, function(
    //   error,
    //   circles
    // ) {
    //   if (error) deffered.reject(error);
    ...
    deffered.resolve(
      oldE.save(user).then(function(e) {
        return Model.populate(e, options.includes || "");
      })
    );
    //   }
