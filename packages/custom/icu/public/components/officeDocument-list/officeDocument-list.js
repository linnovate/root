...
function OfficeDocumentListController(
  $scope,
  $state,
  BoldedService,
  NotifyingService,
  ...
  $scope.entityRowTpl =
    "/icu/components/officeDocument-list/officeDocument-row.html";
  ...
  $scope.create = function(parent) {
    ...
    return OfficeDocumentsService.createDocument(newItem).then(function(
      result
    ) {
...
angular
  .module("mean.icu.ui.officedocumentlist", [])
  .controller("OfficeDocumentListController", OfficeDocumentListController);
