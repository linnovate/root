...
angular
  .module("mean.icu.ui.templateDocdetails", [])
  .controller("TemplateDocDetailsController", TemplateDocDetailsController);

function TemplateDocDetailsController(
  $rootScope,
  $scope,
  $http,
  entity,
  ...
  if (
    ...
    $state.$current.url.source.includes("templateDocs")
  ) {
  ...
  $scope.onCategory = function(value) {
    ...
    TemplateDocsService.updateTemplateDoc($scope.item._id, json).then(
      result => {
        $state.reload();
      }
    );
  ...
  $scope.recycleRestore = function() {
    EntityService.recycleRestore("templateDocs", $scope.item._id).then(
      function() {
        let clonedEntity = angular.copy($scope.item);
