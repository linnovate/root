...
angular
  .module("mean.icu.ui.membersfooter", [])
  .directive("icuMembersFooter", function() {
    function controller(
      ...
      UsersService.getAll().then(users => {
        ...
        for (var i = 0; i < $scope.entity.watchers.length; i++) {
          if (typeof $scope.entity.watchers[i] === "string") {
            $scope.entity.watchers[i] = _.find(users, {
              _id: $scope.entity.watchers[i]
            });
          }
        }
      });
      ...
      function changePerms(member, newPerms) {
        $scope.entity = PermissionsService.changeUsersPermissions(
          $scope.entity,
          member,
          newPerms,
          context
      ...
      $scope.userPermissionStatus = function(member) {
        if (member)
          return (
            PermissionsService.getPermissionStatus(member, $scope.entity) || ""
          );
      };
      ...
      console.log(
        "permissions: " + JSON.stringify($scope.entity.permissions, null, 2)
      );
      ...
      var getWatchersGroups = function() {
        ...
        for (var i = 0; i < groupTypes.length; i++) {
          if ($scope.entity.circles && $scope.entity.circles[groupTypes[i]])
            for (
              var j = 0;
              j < $scope.entity.circles[groupTypes[i]].length;
              j++
            ) {
      ...
      var getNotAssigned = function() {
        ...
        for (var i = 0; i < groupTypes.length; i++) {
          arr3 =
            $scope.entity.circles && $scope.entity.circles[groupTypes[i]]
              ? $scope.entity.circles[groupTypes[i]]
              : [];
      ...
      for (var i = 0; i < $scope.notAssigned.length; i++) {
        if (
          $scope.notAssigned[i] &&
          ($scope.notAssigned[i].job == undefined ||
            $scope.notAssigned[i].job == null)
        ) {
      ...
      var update = function(entity, member, action) {
        ...
        var serviceName =
          serviceMap[$stateParams.id ? context.main : context.entityName];
        ...
        if (entity.serial != undefined) {
          ...
          return service
            .update(entity, data, action, member._id)
            .then(entity => {
              $state.reload();
              return entity;
            });
      ...
      $scope.showMoreWatchers = function() {
        let listWidth = $(".watchersList").width(),
          watcherWidth = 45,
          watchers = _.compact(
            $scope.entity.watchers.concat($scope.watchersGroups)
          ),
          lastIndex = 0;
        ...
        for (let i = 0; i < watchers.length; i++) {
          ...
          if (isLast) {
            $scope.otherWatchers = watchers.slice(
              lastIndex,
              watchers.length - 1
            );
      ...
      $scope.addMember = function(member) {
        ...
        if (member.type) {
          ...
          if (!$scope.entity.circles[member.type])
            $scope.entity.circles[member.type] = [];
        ...
        update($scope.entity, member, "added").then(updatedEntity => {
          ...
          switch (context.main) {
            case "projects":
              ProjectsService.updateWatcher(task, me, backupEntity).then(
                function(result) {
              ...
              break;
            case "tasks":
              TasksService.updateWatcher(task, me, backupEntity).then(function(
                result
              ) {
                ActivitiesService.data.push(result);
              });
              break;
            case "discussions":
              DiscussionsService.updateWatcher(task, me, backupEntity).then(
                function(result) {
            ...
            case "officeDocuments":
              OfficeDocumentsService.updateWatcher(task, me, backupEntity).then(
                function(result) {
                  ActivitiesService.data = ActivitiesService.data || [];
                  ActivitiesService.data.push(result);
                }
              ...
              break;
            case "folders":
              FoldersService.updateWatcher(task, me, backupEntity).then(
                function(result) {
                  ActivitiesService.data = ActivitiesService.data || [];
                  ActivitiesService.data.push(result);
                }
              ...
              break;
            case "offices":
              OfficesService.updateWatcher(task, me, backupEntity).then(
                function(result) {
                  ActivitiesService.data = ActivitiesService.data || [];
                  ActivitiesService.data.push(result);
                }
      ...
      $scope.deleteMember = function(member) {
        if (member.type) {
          $scope.entity.circles[member.type] = _.reject(
            $scope.entity.circles[member.type],
            function(mem) {
          ...
          $scope.entity.watchers = _.reject($scope.entity.watchers, function(
            mem
          ) {
            return member._id === mem._id;
          });
          $scope.entity.permissions = _.reject(
        ...
        switch (context.main) {
          ...
          case "tasks":
            TasksService.updateWatcher(task, me, backupEntity, "remove").then(
              function(result) {
                ActivitiesService.data.push(result);
              }
            );
            break;
          case "discussions":
            DiscussionsService.updateWatcher(
              task,
              me,
              member,
              "removeWatcher"
            ...
            break;
          case "officeDocuments":
            OfficeDocumentsService.updateWatcher(
              task,
              me,
              member,
              "removeWatcher"
