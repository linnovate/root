...
angular
  .module("mean.icu.ui.subtaskslistdirective", [])
  .directive("icuSubTaskList", function(
    $state,
    $uiViewScroll,
    $stateParams,
    $timeout,
      ...
      if (!$scope.displayOnly) {
        if (
          ...
          ($scope.tasks[$scope.tasks.length - 1] &&
            $scope.tasks[$scope.tasks.length - 1]._id)
        ) {
    ...
    return {
      restrict: "EA",
      templateUrl:
        "/icu/components/sub-tasks/sub-task-list.directive.template.html",
      scope: {
        task: "=",
        tasks: "=",
        loadNext: "=",
