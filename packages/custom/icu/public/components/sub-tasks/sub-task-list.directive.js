...
angular.module('mean.icu.ui.subtaskslistdirective', []).directive('icuSubTaskList', function($state, $uiViewScroll, $stateParams, $timeout, UsersService) {
  ...
  function controller($scope, TasksService, UsersService) {
    ...
    $scope.select = function(selectedTask) {
      ...
      $scope.createOrUpdate($scope.tasks[currentTask + 1]).then(function(task) {// $state.go('main.tasks.byparent.details', {
      //     id: task._id,
  ...
  function link($scope, $element) {
    ...
    $scope.isCurrentState = function(id) {
      var isActive = ($state.current.name.indexOf('main.tasks.byentity.details') === 0 || $state.current.name.indexOf('main.tasks.all.details') === 0) && $state.params.id === id;
