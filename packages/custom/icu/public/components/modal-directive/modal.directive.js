"use strict";
angular
  .module("mean.icu.ui.modaldeletetasksbyentity", [])
  .directive("icuOpenModal", function(
    $state,
    $uibModal,
    UsersService,
    ...
    function link(scope, elem, attrs) {
      elem.bind("click", function() {
        if (
          $state.current.name.indexOf("main.tasks.byentity") != -1 &&
          scope.entityName != "Document"
        )
          scope.showModal--;
      ...
      function buildModal() {
        if (
          ...
          var modalInstance = $uibModal.open({
            ...
            size: "md",
            templateUrl:
              "/icu/components/modal-directive/distributedModal.html",
          ...
          var modalInstance = $uibModal.open({
            ...
            size: "lg",
            templateUrl:
              "/icu/components/modal-directive/templateModal/template.html",
            ...
            resolve: {
              templates: function() {
                if (scope.data.folder == undefined) {
                  ...
                  return TemplateDocsService.getTemplatesByFolder(
                    scope.data.folder
                  );
                }
              },
          ...
          var modalInstance = $uibModal.open({
            ...
            size: "lg",
            templateUrl:
              "/icu/components/modal-directive/documentPreview/document-preview.html",
          ...
          var modalInstance = $uibModal.open({
            ...
            size: "40%",
            templateUrl:
              "/icu/components/modal-directive/export-excel-by-date/export-excel-by-date.html",
          ...
          var modalInstance = $uibModal.open({
            ...
            size: "md",
            templateUrl:
              "/icu/components/modal-directive/bulkOperations/bulk-operations.html",
...
function controller($scope, $uibModalInstance, $filter, entity) {
  ...
  $scope.ok = function() {
    if (
      $scope.entity.textDelete &&
      $scope.entity.textDelete == $filter("i18next")("DELETE")
    )
...
function controllerDocument(
  ...
  $scope.ok = function(sendingForm) {
    ...
    if (
      sendingForm.classification == undefined ||
      sendingForm.doneBy == undefined ||
      sendingForm.title == ""
    ) {
      alertify.error("נא למלא הנדון סיווג ולטיפול");
    } else {
      OfficeDocumentsService.sendDocument(
        sendingForm,
        $scope.officeDocument
...
function userCtrl(
  ...
  $scope.notifications = [
    {
      ...
      options: [$filter("i18next")("yes"), $filter("i18next")("no")],
      model: $filter("i18next")(
        $scope.currentUser.GetMailEveryWeekAboutMyTasks
      ),
    ...
    },
    {
      ...
      options: [$filter("i18next")("yes"), $filter("i18next")("no")],
      model: $filter("i18next")(
        $scope.currentUser.GetMailEveryWeekAboutGivenTasks
      ),
    ...
    },
    {
      ...
      options: [$filter("i18next")("yes"), $filter("i18next")("no")],
      model: $filter("i18next")(
        $scope.currentUser.GetMailEveryDayAboutGivenTasks
      ),
  ...
  $scope.ok = function(showUser) {
    ...
    if (
      showUser.classification == undefined ||
      ...
      var resWithDate = res.map(function(r) {
        ...
        if (currentSentTo.length) {
          return Object.assign(
            { date: currentSentTo[0].date, received: true },
            r
          );
        } else {
