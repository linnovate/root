function controllerDocumentPreview($scope, $uibModalInstance, attachment, $filter,$http, UsersService, AttachmentsService, DocumentsService) {
    ...
    $scope.attForFTPPath = $scope.attPath.substring($scope.attPath.indexOf("/files"),$scope.attPath.length);
    ...
    $scope.previewWindow = function (document1) {
        if (document1.documentType == "docx" || document1.documentType == "doc" || document1.documentType == "xlsx" || document1.documentType == "xls" || document1.documentType == "ppt" || document1.documentType == "pptx") {
         ...
         pdfPath = pdfPath.substring(pdfPath.indexOf('preview'),pdfPath.lastIndexOf('.'));
         pdfPath = pdfPath+".pdf";
    ...
    function myTest(document1) {
        ...
        if (document1.documentType == "docx" || document1.documentType == "doc" || document1.documentType == "xlsx" || document1.documentType == "xls" || document1.documentType == "ppt" || document1.documentType == "pptx" || document1.documentType == "tif") {
            var pdfPath = document1.path.replace("/files","/preview");
            ...
            pdfPath=pdfPath.replace(/\//g,"%2f");
            console.log(pdfPath);
            DocumentsService.viewFileLink(pdfPath,document1.documentType).then(function(result){
