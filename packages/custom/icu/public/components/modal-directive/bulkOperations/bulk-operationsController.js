function bulkOperationsController(
  ...
  $scope.selectedTypes = Object.keys($scope.selectedArrays).filter(
    arrayName => $scope.selectedArrays[arrayName].length
  );
  ...
  $scope.statuses = _.intersection(
    ...$scope.selectedTypes.map(type => $scope.statusMap[type])
  );
  ...
  $scope.bulkUpdate = function(
    type,
    value,
    selectedArray = $scope.selectedItems,
    ...
    return MultipleSelectService.bulkUpdate(changedBulkObject, entityName).then(
      result => {
        for (let i = 0; i < selectedArray.length; i++) {
          let entity = result.find(
            entity => entity._id === selectedArray[i]._id
          );
          if (!entity) continue;
        ...
        if (!searchList) MultipleSelectService.setSelectedList(selectedArray);
  ...
  $scope.updateComplex = function() {
    ...
    let updateObject = $scope.selectedWatchers.filter(
      bulkObject => !bulkObject.remove
    );
    ...
    for (let i = 0; i < arraysOfTypes.length; i++) {
      ...
      let removeObject = $scope.selectedWatchers.filter(
        bulkObject => bulkObject.remove
      );
      ...
      MultipleSelectService.bulkUpdate(
        changedBulkObject,
        arraysOfTypes[i] + "s"
      ).then(result => {
        for (let i = 0; i < $scope.selectedItems.length; i++) {
          let entity = result.find(
            entity => entity._id === $scope.selectedItems[i]._id
          );
          if (!entity) continue;
  ...
  $scope.getUnusedWatchers = function() {
    let unUsedIds = _.difference(
      getIdsArray($scope.people),
      getIdsArray($scope.usedWatchers)
    );
    $scope.unusedWatchers = unUsedIds.map(id =>
      _.find($scope.people, { _id: id })
    );
  };

  $scope.transformToBulkObjects = function(array) {
    $scope.selectedWatchers = array.map(object =>
      createBulkWatcher(
        object._id,
        $scope.userPermissionStatus(object),
        false,
        true
      )
    );
  ...
  $scope.getBulkWatchersClass = function(member) {
    let bulkObject = $scope.selectedWatchers.find(
      watcher => watcher._id === member._id
    );
  ...
  $scope.userPermissionStatus = function(member) {
    if (member)
      return PermissionsService.getUnifiedPerms(member, $scope.selectedItems);
    return "No Permissions";
  };

  function changePerms(member, newPerms) {
    let bulkWatcher = $scope.selectedWatchers.find(
      watcher => watcher._id === member._id
    );
  ...
  $scope.addMember = function(member) {
    ...
    let bulkWatcher = createBulkWatcher(
      member._id,
      $scope.userPermissionStatus(member),
      false,
      false
    );
  ...
  $scope.removeMember = function(member) {
    ...
    if (bulkWatcher.primary) {
      ...
      $scope.selectedWatchers = _.reject($scope.selectedWatchers, {
        _id: member._id
      });
    }
  };
  ...
  $scope.showNormalDueInput = () => {
    return $scope.selectedTypes.some(entityType => {
      return (
        ...
        !!$scope.selectedArrays[entityType].length
      );
    });
  };
  $scope.showDiscussionsDueInput = () =>
    !!$scope.selectedArrays["discussion"].length;
  ...
  if ($state.$current.includes["main.search"]) {
    ...
    $scope.availableTags = _.uniq(
      _.flatten($scope.selectedItems.map(item => item.tags))
    );
  } else {
    // For specific entity, suggest tags that exist in other entities
    let serviceName =
      entityName[0].toUpperCase() + entityName.slice(1) + "Service";
  ...
  var initialTags = $scope.selectedItems
    .map(item => item.tags)
    .reduce((acc, i) => {
  ...
  $scope.tagUpdate = function(entityArray, entityName) {
    ...
    MultipleSelectService.bulkUpdate(changedBulkObject, entityName).then(
      result => {
        for (let i = 0; i < $scope.selectedItems.length; i++) {
          let entity = result.find(
            entity => entity._id === $scope.selectedItems[i]._id
