...
angular.module("mean.icu.ui.tabs").directive("icuTabsDocuments", function() {
  function controller(
    $scope,
    $http,
    $state,
            ...
            return $scope.attachments.reduce((promise, file) => {
              return promise.then(() => {
                return DocumentsService.saveAttachments(data, file).then(
                  function(attachment) {
                    AttachmentsService.getAttachmentUser(
                      attachment.creator._id
                    ).then(user => {
    ...
    $scope.checkAttachmentCreator = doc => {
      let havePerms = $scope.havePermissions("tab-content"),
        ...
        isEditor = user =>
          PermissionsService.getPermissionStatus(user, $scope.entity) ===
          "editor";
    ...
    $scope.remove = function(file, index) {
      ...
      DocumentsService.delete(file._id, {
        parent: parent,
        id: $scope.entity._id
      })
    ...
    $scope.autoFormatFilesize = function(fileSize) {
      if (fileSize > 1000000000) {
        return (fileSize / 1000000000.0).toPrecision(3) + " GB";
      } else if (fileSize > 1000000) {
        return (fileSize / 1000000.0).toPrecision(3) + " MB";
      } else if (fileSize > 1000) {
        return (fileSize / 1000.0).toPrecision(3) + " KB";
