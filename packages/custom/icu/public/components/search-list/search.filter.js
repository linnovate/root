  ...
  .filter("filteringByUpdated", function(SearchService, $location) {
    return function(results) {
      ...
      let filteringResults = SearchService.filteringResults.map(function(e) {
        ...
        let res = false;
        if (e._type === "officeDocument") e.due = e.created;
        if (SearchService.filteringByDueDate && e.due) {
          ...
          let filterStart = new Date(
            SearchService.filterDateOption.startDate._d ||
              SearchService.filterDateOption.startDate
          );
          let filterEnd = new Date(
            SearchService.filterDateOption.endDate._d ||
              SearchService.filterDateOption.endDate
          );
          if (
            entityStartDate >= filterStart &&
            entityEndDate <= filterEnd &&
            entityDate > filterDate
          ) {
            res = true;
          } else res = false;
        } else if (entityDate > filterDate) res = true;
        return res ? e.id : -1;
      });

      let filteredResults = results.filter(
        entity => filteringResults.indexOf(entity.id) > -1
      );
      for (let i = 0; i < filteredResults.length; i++) {
        filteredResults[i].index = filteredResults.findIndex(res => {
          return (
            (res.id || res._id) ===
            (filteredResults[i].id || filteredResults[i]._id)
          );
        });
      }
