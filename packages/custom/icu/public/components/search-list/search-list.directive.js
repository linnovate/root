  ...
  .directive("icuSearchList", function(LayoutService) {
    function controller(
      ...
      $scope.initialize = function($event, result) {
        ...
        $state.go(
          `main.search.${result.type || result._type}.${
            window.config.defaultTab
          }`,
          {
      ...
      for (var i = 0; i < $scope.results.length; i++) {
        $scope.results[i]._id = $scope.results[i].id =
          $scope.results[i].id || $scope.results[i]._id;
      }
      ...
      function multipleSelectRefreshState() {
        ...
        $scope.cornerState = MultipleSelectService.refreshCornerState(
      ...
      function refreshActiveItemsInList() {
        ...
        for (let filteredResult of filteredResults) {
          let entity = $scope.selectedItems.find(
            selected => selected._id === filteredResult._id
          );
      ...
      function multipleSelectSetAllSelected(status) {
        ...
        for (let i = 0; i < filteredResults.length; i++) {
          ...
          if (!rowSelectStatus.selected)
            MultipleSelectService.refreshSelectedList(filteredResults[i]);
