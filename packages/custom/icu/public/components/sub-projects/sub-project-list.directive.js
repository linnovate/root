...
angular
  .module("mean.icu.ui.subprojectslistdirective", [])
  .directive("icuSubProjectList", function(
    $state,
    $uiViewScroll,
    $stateParams,
    $timeout,
    ...
    function controller($scope, ProjectsService, UsersService) {
      ...
      if (!$scope.displayOnly) {
        if (
          ...
          ($scope.projects[$scope.projects.length - 1] &&
            $scope.projects[$scope.projects.length - 1]._id)
        ) {
      ...
      $scope.select = function(selectedProject) {
        ...
        $scope
          .createOrUpdate($scope.projects[currentProject + 1])
      ...
      $scope.initDue = function(project) {
        if (project.due) project.due = new Date(project.due);
      };
        ...
        return {
          ...
          onClose: function() {
            if ($scope.checkDate(project)) {
              document.getElementById("ui-datepicker-div").style.display =
                "block";
              $scope.open(project);
            } else {
              document.getElementById("ui-datepicker-div").style.display =
    ...
    function link($scope, $element) {
      ...
      $scope.isCurrentState = function(id) {
        var isActive =
          ($state.current.name.indexOf("main.projects.byentity.details") ===
            0 ||
    ...
    return {
      restrict: "EA",
      templateUrl:
        "/icu/components/sub-projects/sub-project-list.directive.template.html",
      scope: {
        project: "=",
        projects: "=",
        loadNext: "=",
