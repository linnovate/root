...
function EntityListController(
  ...
  let service = $injector.get(
    context.main[0].toUpperCase() + context.main.slice(1) + "Service"
  );
  ...
  $scope.isCurrentEntityState = function(id) {
    return (
      $state.current.name.indexOf(
        `main.${$scope.$parent.entityName}.byentity`
      ) === 0 && $state.current.name.indexOf("details") === -1
    );
  };
  ...
  $scope.isCurrentState = function(id) {
    var isActive =
      ($state.current.name.indexOf(
        `main.${$scope.$parent.entityName}.byparent.details`
      ) === 0 ||
        $state.current.name.indexOf(
          `main.${$scope.$parent.entityName}.byentity.details`
        ) === 0 ||
        $state.current.name.indexOf(
          `main.${$scope.$parent.entityName}.all.details`
        ) === 0) &&
      $state.params.id === id;
  ...
  function scrollToElement() {
    ...
    if (elementBottom > listBottom) elem.scrollIntoView({ behavior: "smooth" });
  }
  ...
  function getParentName() {
    ...
    $scope.parentName =
      parentEntity && (parentEntity.title || parentEntity.name);

    if (!$scope.parentName) {
      EntityService.getByEntityId(
        context.entityName + "s",
        context.entityId
      ).then(entity => {
  ...
  $scope.activeToggle = {
    field: !EntityService.isActiveStatusAvailable()
      ? "all"
      : $stateParams.activeToggle || "active",
  ...
  $scope.sorting = {
    field: $scope.sortingList.find(f =>
      [f.value, f.title].includes($stateParams.sort.replace(/^-/, ""))
    ),
  ...
  $scope.excel = {
    ...
    officeList: _.filter(
      $scope.offices,
      office => office.title && office.title !== ""
    )
  };
  ...
  $scope.onCreate = function() {
    if (
      $state.current.name.indexOf("byentity") !== -1 ||
      $state.current.name.indexOf("byparent") !== -1
    ) {
      var parent = {};
  ...
  $scope.$on("changeCornerState", (event, cornerState) =>
  ...
  function getRefreshedCornerState() {
    return MultipleSelectService.refreshCornerState(
      filterResults($scope.visibleItems).length
    );
  }

  function refreshActiveItemsInList() {
    for (let item of $scope.visibleItems) {
      let entity = $scope.selectedItems.find(
        selectedItems => selectedItems._id === item._id
      );
    ...
    function loadWithVisibleCheck(listEnd, loadedVisibleCount) {
      if (!$scope.isLoading && loadedVisibleCount < 25 && !listEnd) {
        ...
        let alreadyLoaded = Boolean(
          $scope.items.find(item => item._id === $stateParams.id)
        );
        ...
        let isActive = Boolean(
          $scope.visibleItems.find(item => item._id === $stateParams.id)
        );
  ...
  function setStatusFilterValue(value) {
    ...
    if (!value) value = "all";
    if (["new", "assigned", "in-progress", "review"].includes(value))
      newActiveToggleField = "active";
    if (["rejected", "done", "archived", "canceled", "done"].includes(value))
      newActiveToggleField = "nonactive";
  ...
  function filterResults(itemsArray) {
    ...
    newArray = $filter("filterByActiveStatus")(
      newArray,
    ...
    newArray = $filter("orderBy")(
      newArray,
      $scope.sorting.field.value,
      $scope.sorting.isReverse
    );
  ...
  $scope.havePermissions = (entity, type) =>
    PermissionsService.havePermissions(entity, type);
  $scope.haveEditiorsPermissions = entity =>
    PermissionsService.haveEditorsPerms(entity);
  $scope.permsToSee = entity => PermissionsService.haveAnyPerms(entity);
}

angular
  .module("mean.icu.ui.entityList", ["dragularModule"])
