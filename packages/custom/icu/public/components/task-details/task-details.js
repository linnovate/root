...
angular
  .module("mean.icu.ui.taskdetails", [])
...
function TaskDetailsController(
  ...
  $scope.onStar = function(value) {
    TasksService.updateStar($scope.item, me, backupEntity).then(function(
      result
    ) {
  ...
  $scope.onDateDue = function(value) {
    ...
    TasksService.updateDue($scope.item, me, backupEntity).then(function(
      result
    ) {
      backupEntity = angular.copy($scope.item);
      ActivitiesService.data.push(result);
  ...
  $scope.onStatus = function(value) {
    ...
    TasksService.updateStatus($scope.item, me, backupEntity).then(function(
      result
    ) {
      backupEntity = angular.copy($scope.item);
      ActivitiesService.data.push(result);
  ...
  $scope.onTags = function(value) {
    ...
    TasksService.updateTags($scope.item, me, backupEntity).then(function(
      result
    ) {
      backupEntity = angular.copy($scope.item);
      ActivitiesService.data.push(result);
  ...
  $scope.recycle = function() {
    TasksService.removeFromParent($scope.item).then(() => {
      EntityService.recycle("tasks", $scope.item._id).then(function() {
        ...
        TasksService.updateStatus(clonedEntity, me, $scope.item).then(function(
          result
        ) {
          ActivitiesService.data.push(result);
        });
  ...
  $scope.recycleRestore = function() {
    ...
    TasksService.addToParent(entity).then(() => {
      EntityService.recycleRestore("tasks", entity._id).then(function() {
        ...
        // just for activity status
        TasksService.updateStatus(clonedEntity, me, entity).then(function(
          result
        ) {
          ActivitiesService.data.push(result);
        });
  ...
  $scope.duplicate = function() {
    ...
    let duplicate = _.pick($scope.item, ["title", "description", "due"]);

    Object.assign(newItem, duplicate);
    TasksService.create(newItem).then(result => {
    ...
    return TasksService.template2subTasks(id, {
      ...
      result.forEach(subTask => {
        ...
        TasksService.create(subTask).then(result => {
          result.isNew = true;
          $scope.item.subTasks.splice(
            $scope.item.subTasks.length - 1,
            0,
            result
          );
          $timeout(function() {
  ...
  $scope.update = function(item, type, proj) {
    ...
    if (type === "discussion" && item.discussion) {
      ...
      item.permissions = unionById(
        item.permissions,
        item.discussion.permissions
      );
    }
