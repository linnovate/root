  ...
  .directive("icuDisplayBy", function() {
    function controller(
      ...
      $scope.statusListChange = function(type) {
        ...
        if ($scope.currentType == "active") {
          if ($scope.activeList.indexOf(type) < 0) $scope.activeList.push(type);
          else {
      ...
      $scope.isActive = function(type, status) {
        if (
          status !== "active" &&
          status !== "nonactive" &&
          $scope.tmpStatus.length &&
          $scope.tmpStatus.indexOf(type) > -1
        )
          return true;
        if (status !== "active" && status !== "nonactive" && status == type)
          ...
          if ($scope.currentType == "all") {
            if ($scope.userFilterList.indexOf(type) > -1) return true;
          }
          if ($scope.currentType == "active") {
            if ($scope.activeList.indexOf(type) > -1) return true;
          }
          if ($scope.currentType == "nonactive") {
            if ($scope.archiveList.indexOf(type) > -1) return true;
          }
        }
        return false;
      };
      ...
      $scope.showM = function(type) {
        ...
        if (type == "empty") {
          ...
          $scope.AllStatus = arrayUnique(
            $scope.activeList.concat($scope.archiveList)
          );
        $scope.tmpStatus = [];
      };
          ...
          .then(() => {
            ...
            return OfficesService.getAll(0, 0, "created").then(offices => {
              ...
              $scope.offices.forEach(function(office) {
                if (office.title) $scope.officesList.push(office);
              });
            });
          })
          .then(() => {
      ...
      NotifyingService.subscribe(
        ...
        function() {
          ...
          OfficeDocumentsService.getAll(0, 25, "-created").then(function(data) {
            ...
            $scope.officeDocumentsList = $scope.officeDocuments.filter(
              filterEntities
            );
          });
      ...
      $scope.getLinkUrl = function() {
        return $state.href("main." + $scope.activeTab.state, {
    ...
    function link($scope, $element, context) {
      ...
      $scope.collapse = function(entityName) {
        $scope.displayLimit[entityName] =
          $scope.displayLimit.default[entityName];
      };
    }
