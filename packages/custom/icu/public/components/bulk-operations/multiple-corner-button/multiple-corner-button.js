...
angular
  ...
  .directive("multipleCornerButton", function() {
    function controller($rootScope, $scope, MultipleSelectService) {
      $scope.selectedItems =
        $scope.$parent.selectedItems || MultipleSelectService.getSelected();
      $scope.cornerState =
        $scope.$parent.cornerState || MultipleSelectService.getCornerState();

      $scope.notifyChangingState = () =>
        $rootScope.$broadcast(
          "changeCornerState",
          MultipleSelectService.changeCornerState()
        );
    ...
    return {
      ...
      templateUrl:
        "/icu/components/bulk-operations/multiple-corner-button/multiple-corner-button.html"
    };
  });
