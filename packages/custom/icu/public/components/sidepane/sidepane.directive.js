...
angular.module("mean.icu.ui.sidepane", []).directive("icuSidepane", function() {
  function controller(
    $scope,
    $state,
    $stateParams,
    ...
    $scope.officeDocuments =
      $scope.officeDocuments.data || $scope.officeDocuments;
    ...
    $scope.filteringData = {
      issue:
        $location.$$search && $location.$$search.type
    ...
    let getEntitiesAndWatchers = function(filteredByType) {
      for (let i = 0; i < filteredByType.length; i++) {
        ...
        if (
          filteredByType[i].discussions &&
          filteredByType[i].discussions.length
        )
          $scope.filteringData.discussions.push(
            filteredByType[i].discussions[0]
          );
        ...
        if (filteredByType[i].watchers && filteredByType[i].watchers.length)
          $scope.filteringData.watchers = $scope.filteringData.watchers.concat(
            filteredByType[i].watchers
          );
      }
    ...
    $scope.filterSearchByType = function(flag) {
      ...
      for (let i = 0; i < results.length; i++) {
        if (
          results[i]._type === $scope.filteringData.issue ||
          $scope.filteringData.issue === "all"
        ) {
          if ($rootScope.status) {
    ...
    let getTruth = function(obj) {
      // return truth value in a single object
      let arr = [];
      for (let key in obj) {
        if (obj[key]) {
          arr.push(key);
    ...
    $scope.filterSearchByEntity = function() {
      ...
      let discussions = getTruth(
        $scope.filteringData.selectedEntities.discussions
      );
      ...
      for (let i = 0; i < filteringResults.length; i++) {
        if (
          (filteringResults[i].project &&
            projects.indexOf(filteringResults[i].project) > -1) ||
          (filteringResults[i].folder &&
            folders.indexOf(filteringResults[i].folder) > -1) ||
          (filteringResults[i].office &&
            offices.indexOf(filteringResults[i].office) > -1)
        )
          filteredByEntity.push(filteringResults[i]);
        if (
          filteringResults[i].discussions &&
          filteringResults[i].discussions.length &&
          discussions.indexOf(filteringResults[i].discussions[0]) > -1
        )
          filteredByEntity.push(filteringResults[i]);
      }
