 ...
 */
angular
  .module("mean.icu.ui.discussiondetails")
  .directive("discussionDue", DiscussionDue);
...
function DiscussionDue() {
  return {
    ...
    link: link,
    templateUrl:
      "/icu/components/discussion-details/discussion-due/discussion-due.html",
    restrict: "E"
  };
  ...
  function link($scope, element, attrs) {
    ...
    $(document).click(function(event) {
      if (
        !$(event.target).closest(".dueDiv").length &&
        !$(event.target).closest(".detail-due").length
      ) {
        if ($scope.fade) {
          $scope.dueClicked();
        }
    ...
    $scope.startDueOptions = {
      ...
      onClose: function() {
        let datePicker = $scope.inModal
          ? $("#modalBulk").find("ui-datepicker-div")
          : $("ui-datepicker-div");
    ...
    $scope.checkDate = function() {
      ...
      d.setHours(0, 0, 0, 0);
      if (
        d > $scope.item.startDate ||
        d > $scope.item.endDate ||
    ...
    $scope.dueClicked = function() {
      let dueDiv = $scope.inModal
        ? $("#modalBulk").find(".dueDiv")
        : $(".dueDiv");
      if (!$scope.fade) {
        dueDiv.fadeIn(1000);
      } else {
    ...
    $scope.updateDatesString = function() {
      ...
      if ($scope.item.startDate) {
        var startStr =
          $scope.item.startDate.getDate() +
          "/" +
          ($scope.item.startDate.getMonth() + 1) +
          "/" +
          $scope.item.startDate.getFullYear();
      ...
      if ($scope.item.allDay) {
        ...
        if ($scope.item.startTime) {
          var ho =
            $scope.item.startTime.getHours().toString().length == 1
              ? "0" + $scope.item.startTime.getHours().toString()
              : $scope.item.startTime.getHours().toString();
          var min =
            $scope.item.startTime.getMinutes().toString().length == 1
              ? "0" + $scope.item.startTime.getMinutes().toString()
              : $scope.item.startTime.getMinutes().toString();
          startStr = ho + ":" + min;
          $scope.firstStr = $scope.item.startDate
            ? $scope.firstStr + " " + startStr
            : "";
        }
        if ($scope.item.endDate) {
          ...
          var endStr =
            $scope.item.endDate.getDate() +
            "/" +
            ($scope.item.endDate.getMonth() + 1) +
            "/" +
            $scope.item.endDate.getFullYear();
          $scope.secondStr = endStr;
          if ($scope.item.endTime) {
            var ho =
              $scope.item.endTime.getHours().toString().length == 1
                ? "0" + $scope.item.endTime.getHours().toString()
                : $scope.item.endTime.getHours().toString();
            var min =
              $scope.item.endTime.getMinutes().toString().length == 1
                ? "0" + $scope.item.endTime.getMinutes().toString()
                : $scope.item.endTime.getMinutes().toString();
            endStr = ho + ":" + min;
