...
function DiscussionDetailsController(
  $scope,
  $rootScope,
  entity,
  tasks,
  ...
  if (
    ...
    $state.$current.url.source.includes("discussions")
  ) {
  ...
  $scope.recycle = function() {
    EntityService.recycle("discussions", $scope.item._id).then(function() {
      ...
      DiscussionsService.updateStatus(clonedEntity, $scope.item).then(function(
        result
      ) {
  ...
  $scope.recycleRestore = function() {
    EntityService.recycleRestore("discussions", $scope.item._id).then(
      function() {
        let clonedEntity = angular.copy($scope.item);
        clonedEntity.status = "un-deleted";
        // just for activity status
        DiscussionsService.updateStatus(clonedEntity, $scope.item).then(
          function(result) {
            ActivitiesService.data.push(result);
          }
        );
  ...
  $scope.summary = function(discussion) {
    ...
    if (
      !(
        discussion.allDay ||
        (discussion.startTime && discussion.endDate && discussion.endTime)
      )
    ) {
      alert("Dates problem");
      return;
  ...
  $scope.update = function(discussion, type) {
    ...
    switch (type) {
      case "startDate":
        DiscussionsService.updateStartDate(
          discussion,
          $scope.me,
          backupEntity
      ...
      case "endDate":
        DiscussionsService.updateEndDate(
          discussion,
          $scope.me,
          backupEntity
        ).then(function(result) {
      ...
      case "star":
        DiscussionsService.updateStar(discussion, $scope.me, backupEntity).then(
          function(result) {
            backupEntity = angular.copy($scope.item);
            ActivitiesService.data = ActivitiesService.data || [];
            ActivitiesService.data.push(result);
      ...
      case "tags":
        DiscussionsService.updateTags(discussion, $scope.me, backupEntity).then(
          function(result) {
            backupEntity = angular.copy($scope.item);
            ActivitiesService.data = ActivitiesService.data || [];
            ActivitiesService.data.push(result);
      ...
      case "status":
        DiscussionsService.updateStatus(
          discussion,
          $scope.me,
          backupEntity
        ).then(function(result) {
      ...
      case "location":
        DiscussionsService.updateLocation(
          discussion,
          $scope.me,
          backupEntity
        ).then(function(result) {
      ...
      case "assign":
        ...
        DiscussionsService.updateAssign(
          discussion,
          $scope.me,
          backupEntity
        ).then(function(result) {
        ...
        break;
      case "description":
        DiscussionsService.updateDescription(
          discussion,
          $scope.me,
          backupEntity
        ).then(function(result) {
        ...
        break;
      case "title":
        DiscussionsService.updateTitle(
          discussion,
          $scope.me,
          backupEntity
        ).then(function(result) {
  ...
  $scope.updateAndNotify = function(item) {
    ...
    DiscussionsService.update(item).then(function(result) {
      ...
      DiscussionsService.updateAssign(item, $scope.me, backupEntity).then(
        function(res) {
  ...
  $scope.havePermissions = function(type, enableRecycled) {
    ...
    return (
      PermissionsService.havePermissions($scope.entity, type) && enableRecycled
    );
  };
