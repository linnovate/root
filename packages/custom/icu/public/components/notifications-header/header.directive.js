...
angular
  .module("mean.icu.ui.notificationsheader", [])
  .directive("icuNotificationsHeader", function(
    ...
    function controller($scope) {
        ...
        TasksService.create(task)
          ...
          .then(item => {
            ...
            if (params.entity === "project") {
              ProjectsService.getById(params.entityId)
                .then(parentEntity => {
      ...
      $scope.createOfficeDocument = function() {
        ...
        OfficeDocumentsService.createDocument(newDocument).then(function(
          result
        ) {
  ...
  .filter("when", function($filter) {
    return function(input) {
      ...
      return (
        $filter("i18next")($filter("date")(input, "MMMM")) +
        " " +
        d +
        $filter("i18next")(dd) +
        $filter("date")(input, ", yyyy") +
        $filter("date")(input, ", hh:mm")
      );
    };
  })
