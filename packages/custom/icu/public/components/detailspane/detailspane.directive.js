...
angular
  .module("mean.icu.ui.detailspane", [])
  .directive("icuDetailspane", function($uibModal) {
    function controller($scope, NotifyingService, MultipleSelectService) {
      NotifyingService.subscribe(
        "multipleDisableDetailsPaneCheck",
        function() {
    ...
    function link($scope, $element, $attr) {
      var changeHeight = function(event) {
        ...
        if (target.parents(".tabs").length || target.hasClass("tabs")) {
          return;
        }

        var height =
          target.parents(".tab-content").length ||
...
angular.module("mean.icu.ui.detailspane").directive("detailsView", function() {
...
angular
  .module("mean.icu")
  .controller("DetailsPaneModalInstanceCtrl", function($uibModalInstance) {
    function ok() {
