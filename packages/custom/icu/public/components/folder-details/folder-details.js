...
angular
  .module("mean.icu.ui.folderdetails", [])
  .controller("FolderDetailsController", FolderDetailsController);

function FolderDetailsController(
  $rootScope,
  $scope,
  entity,
  me,
  ...
  if (
    ...
    $state.$current.url.source.includes("folders")
  ) {
  ...
  FoldersService.getTags().then(result => ($scope.tags = result));
  ...
  $scope.recycle = function() {
    EntityService.recycle("folders", $scope.item._id).then(function() {
      ...
      FoldersService.updateStatus(clonedEntity, $scope.item).then(function(
        result
      ) {
  ...
  $scope.onCategory = function(value) {
    if (value) {
      ...
      $scope.item.watchers = unionById(
        $scope.item.watchers,
        $scope.item.office.watchers
      );
      $scope.item.permissions = unionById(
        $scope.item.permissions,
        $scope.item.office.permissions
      );
    } else {
      $scope.item.office = {};
    }
  ...
  $scope.update = function(folder, context) {
    ...
    FoldersService.update(folder, context).then(function(res) {
      ...
      switch (context.name) {
        case "status":
          FoldersService.updateStatus(folder, me, backupEntity).then(function(
            result
          ) {
            backupEntity = angular.copy($scope.item);
        ...
        case "star":
          FoldersService.updateStar(folder, me, backupEntity).then(function(
            result
          ) {
            backupEntity = angular.copy($scope.item);
            ActivitiesService.data = ActivitiesService.data || [];
          ...
          break;
        case "title":
          FoldersService.updateTitle(folder, me, backupEntity).then(function(
            result
          ) {
            backupEntity = angular.copy($scope.item);
            ActivitiesService.data = ActivitiesService.data || [];
          ...
          break;
        case "description":
          FoldersService.updateDescription(folder, me, backupEntity).then(
            function(result) {
              backupEntity = angular.copy($scope.item);
              ActivitiesService.data = ActivitiesService.data || [];
              ActivitiesService.data.push(result);
