...
angular
  .module("mean.icu.ui.tasklistdirective", ["dragularModule"])
  .directive("icuTaskList", function(
    ...
    function controller(
      ...
      if (!$scope.displayOnly) {
        if (context.entityName === "my") {
          ...
          if (context.entityName === "task") {
            ...
            $scope.tasks.push(_(newTask).clone());
          } else $scope.tasks.push(_(newTask).clone());
        }
      }
      ...
      $scope.select = function(selectedTask) {
        ...
        $scope
          .createOrUpdate($scope.tasks[currentTask + 1])
          .then(function(task) {
    ...
    function link($scope, $element) {
      ...
      $scope.isCurrentState = function(id) {
        var isActive =
          ($state.current.name.indexOf("main.tasks.byparent.details") === 0 ||
    ...
    return {
      restrict: "A",
      templateUrl:
        "/icu/components/task-list-directive/task-list.directive.template.html",
      scope: {
        tasks: "=",
        loadNext: "=",
        loadPrev: "=",
