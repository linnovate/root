...
angular
  ...
  .service("EntityService", function(
    ...
    function getEntityActivityStatus(filterValue, entityType, entityStatus) {
      return SettingServices.getIsActiveStatus(
        filterValue,
        entityType,
        entityStatus
      );
    ...
    function recycleRestore(type, id) {
      return $http
        .patch(ApiUri + "/" + type + "/" + id + "/recycle_restore")
        .then(function(result) {
          WarningsService.setWarning(result.headers().warning);
          return result.data;
        });
    ...
    function getRecycleBin(type) {
      return $http
        .get(ApiUri + "/" + type + "/get_recycle_bin")
        .then(function(result) {
          WarningsService.setWarning(result.headers().warning);
          return result.data;
        });
