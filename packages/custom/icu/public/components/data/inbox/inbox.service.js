...
angular
  .module("mean.icu.data.inboxservice", [])
  .service("InboxService", function(
    ApiUri,
    $http,
    $stateParams,
    $i18next,
    ...
    function getUpdateEntities(activities) {
      return $http.post(ApiUri + EntityPrefix, activities).then(
        result => {
    ...
    function getActivityDescription(activity) {
      ...
      switch (activity.updateField) {
        ...
        case "tags":
          ...
          break;
        case "due":
          return `${creator} ${$i18next("changedDueDateTo")} ${moment(
            activity.current
          ).format("DD/MM/YYYY")}`;
          break;
        case "startDate":
          return `${creator} ${$i18next("changedStartDateTo")} ${moment(
            activity.current
          ).format("DD/MM/YYYY")}`;
          break;
        case "endDate":
          return `${creator} ${$i18next("changedEndDateTo")} ${moment(
            activity.current
          ).format("DD/MM/YYYY")}`;
          break;
        case "status":
          return `${creator} ${$i18next("changedStatusTo")} ${$i18next(
            activity.current
          )}`;
          break;
        case "title":
          ...
          break;
        case "assign":
          let currentUser = activity.current
            ? getUser(activity.current._id || activity.current)
            : null;
          let prevUser = activity.prev
            ? getUser(activity.prev._id || activity.prev)
            : null;

          if (currentUser && prevUser)
            return `${creator} ${$i18next(
              "assignedUser"
            )} ${currentUser} ${$i18next("and")} ${$i18next(
              "unassign"
            )} ${prevUser}`;
          else if (currentUser)
          ...
          break;
        case "location":
          return `${creator} ${$i18next("changedLocationTo")} ${
            activity.current
          }`;
          break;
        case "color":
          ...
          break;
        case "description":
          return `${creator} ${$i18next("updatedDescription")} ${
            activity.current
          }`;
          break;
        case "comment":
        ...
        case "attachment":
          if ("current" in activity) {
            return `${creator} ${$i18next("addedAttachment")} <strong>${
              activity.current
            }</strong>`;
          } else {
