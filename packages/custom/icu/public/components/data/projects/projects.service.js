...
angular
  .module("mean.icu.data.projectsservice", [])
  .service("ProjectsService", function(
    ...
    function getSubProjects(projectId) {
      return $http
        .get(ApiUri + EntityPrefix + "/subprojects/" + projectId)
        .then(function(result) {
          WarningsService.setWarning(result.headers().warning);
          return result.data;
        });
    ...
    function update(project, context) {
      ...
      return $http
        .put(ApiUri + EntityPrefix + "/" + project._id, project)
        .then(function(result) {
          WarningsService.setWarning(result.headers().warning);
          if (
            project.subProjects &&
          ...
          if (result.data && result.data.subProjects)
            for (var i = 0; i < result.data.subProjects.length; i++) {
              if (result.data.subProjects[i].due) {
                result.data.subProjects[i].due = new Date(
                  result.data.subProjects[i].due
                );
              }
            }
        ...
        .then(entity => {
          let bolded = _.pick(
            BoldedService.boldedUpdate(entity, "projects", "update"),
            "bolded"
          );
    ...
    function star(project) {
      return $http
        .patch(ApiUri + EntityPrefix + "/" + project._id + "/star", {
          star: !project.star
        })
        .then(function(result) {
          WarningsService.setWarning(result.headers().warning);
    ...
    function WantToCreateRoom(project) {
      return $http
        .post(
          ApiUri + EntityPrefix + "/" + project._id + "/WantToCreateRoom",
          project
        )
        .then(function(result) {
          WarningsService.setWarning(result.headers().warning);
    ...
    function template2subProjects(templateId, data) {
      return $http
        .post(ApiUri + "/templates/" + templateId + "/toSubProjects", data)
        .then(function(result) {
          WarningsService.setWarning(result.headers().warning);
          return result.data;
        });
