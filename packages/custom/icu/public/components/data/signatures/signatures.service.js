...
angular
  .module("mean.icu.data.signaturesservice", [])
  .service("SignaturesService", function(
    ApiUri,
    $http,
    PaginationService,
    TasksService,
    ...
    function getByOfficeId(id) {
      return $http
        .get(ApiUri + EntityPrefix + "/getByOfficeId/" + id)
    ...
    function create(signature) {
      return $http
        .post(ApiUri + EntityPrefix + "/create", signature)
        .then(function(result) {
          WarningsService.setWarning(result.headers().warning);
