...
angular
  .module("mean.icu.data.permissionsservice", [])
  .service("PermissionsService", function(
    ...
    function changeUsersPermissions(entity, user, perms, context) {
      ...
      if (clonedEntity.serial != undefined) {
        ...
        serviceName.updateWatcherPerms(
          clonedEntity,
          user,
          clonedEntity.permissions
        );
      } else {
    ...
    function updateEntity(entity, context) {
      var entityService =
        context.main === "tasks" ? TasksService : ProjectsService;
      ...
      if (context.main !== "discussions") {
        ...
        DiscussionsService.updateAssign(entity, backupEntity).then(function(
          result
        ) {
    ...
    function permissions(entity, type, user) {
      ...
      if (entity.permissions.length !== 0) {
        ...
        switch (usersPerms.level) {
          case "editor":
            havePerm = editorPerms[type];
            break;
          case "commenter":
            havePerm = commenterPerms[type];
            break;
          case "viewer":
            havePerm = viewerPerms[type];
            break;
        }
      }
      return havePerm;
    ...
    function getUnifiedPerms(member, selectedItems) {
      ...
      for (let i = 0; i < selectedItemsLength; i++) {
        let perm = selectedItems[i].permissions.filter(
          perm => perm.id === member._id
        );
        if (perm[0] && unifPerms !== perm[0].level)
          unifPerms = "Different permissions";
      }
