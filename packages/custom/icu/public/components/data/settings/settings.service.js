...
angular
  .module("mean.icu.data.settingsservice", [])
  .service("SettingServices", function($http, WarningsService) {
    ...
    let activeStatusConfigured =
      config.activeStatus && config.activeStatus.nonActiveStatus ? true : false;
    let configNonActiveStatus =
      config.activeStatus && config.activeStatus.nonActiveStatus
        ? config.activeStatus.nonActiveStatus
        : null;

    function getActiveStatuses(type) {
      configNonActiveStatus = configNonActiveStatus
        ? configNonActiveStatus
        : statusList.Typed(type);
      let nonActiveStatus = new Set(Object.values(configNonActiveStatus));
      let typeStatusLowerCase = statusList
        .Typed(type)
    ...
    function getIsActiveStatus(filter, type, status) {
      ...
      switch (filter) {
        default:
        case "active":
          arr = getActiveStatuses(type).find(
            element => typeStatusLowerCase == element
          );
          return arr ? true : false;
          break;
        case "nonactive":
          arr = getNonActiveStatuses(type).find(
            element => typeStatusLowerCase == element
          );
          return arr ? true : false;
          break;
