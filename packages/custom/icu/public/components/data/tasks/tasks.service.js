...
angular
  .module("mean.icu.data.tasksservice", [])
  .service("TasksService", function(
    ...
    function removeFromParent(entity) {
      return new Promise(function(resolve) {
        if (entity.parent) {
          getById(entity.parent).then(parent => {
    ...
    function relateToTask(taskId, entityType, entityId) {
      return $http
        .post(ApiUri + EntityPrefix + "/relate", {
          taskId: taskId,
    ...
    function search(term) {
      return $http
        .get(ApiUri + "/search?term=" + term + "&index=task")
        .then(function(result) {
          WarningsService.setWarning(result.headers().warning);
    ...
    function update(task, data) {
      ...
      if (
        ...
        !task.subTasks[task.subTasks.length - 1]._id
      ) {
        var subTask = task.subTasks[task.subTasks.length - 1];
      }

      return $http
        .put(ApiUri + EntityPrefix + "/" + task._id, task)
        .then(function(result) {
          WarningsService.setWarning(result.headers().warning);
          for (var i = 0; i < result.data.subTasks.length; i++) {
            if (result.data.subTasks[i].due) {
              result.data.subTasks[i].due = new Date(
                result.data.subTasks[i].due
              );
            }
          }
        ...
        .then(entity => {
          let bolded = _.pick(
            BoldedService.boldedUpdate(entity, "tasks", "update"),
            "bolded"
          );
    ...
    function star(task) {
      return $http
        .patch(ApiUri + EntityPrefix + "/" + task._id + "/star", {
          star: !task.star
        })
        .then(function(result) {
          WarningsService.setWarning(result.headers().warning);
    ...
    function getStarredByassign() {
      return $http
        .get(ApiUri + EntityPrefix + "/starred/byAssign")
        .then(function(result) {
          WarningsService.setWarning(result.headers().warning);
          return result.data;
        });
    ...
    function getSubTasks(taskId) {
      return $http
        .get(ApiUri + EntityPrefix + "/subtasks/" + taskId)
        .then(function(result) {
          WarningsService.setWarning(result.headers().warning);
          return result.data;
        });
    ...
    function saveTemplate(id, name) {
      return $http
        .post(ApiUri + EntityPrefix + "/" + id + "/toTemplate", name)
        .then(function(result) {
          WarningsService.setWarning(result.headers().warning);
          return result.data;
        })
    ...
    function template2subTasks(templateId, data) {
      return $http
        .post(ApiUri + "/templates/" + templateId + "/toSubTasks", data)
        .then(function(result) {
          WarningsService.setWarning(result.headers().warning);
          return result.data;
        });
    ...
    function MyTasksOfNextWeekSummary() {
      return $http
        .post(ApiUri + EntityPrefix + "/MyTasksOfNextWeekSummary")
        .then(function(result) {
          WarningsService.setWarning(result.headers().warning);
          return result.data;
        });
    ...
    function GivenTasksOfNextWeekSummary() {
      return $http
        .post(ApiUri + EntityPrefix + "/GivenTasksOfNextWeekSummary")
        .then(function(result) {
          WarningsService.setWarning(result.headers().warning);
          return result.data;
        });
    ...
    function excel() {
      let { entity, entityId } = $state.params;
      console.log("something something something");
      const reqUrl =
        entity && entityId
          ? `${ApiUri}/tasks/excel?entity=${entity}&entityId=${entityId}`
          : `${ApiUri}/tasks/excel`;
      console.log(reqUrl);
      $http
        .get(reqUrl, {
