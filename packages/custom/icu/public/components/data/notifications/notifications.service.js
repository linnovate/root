...
angular.module('mean.icu.data.notificationsservice', [])
    .service('NotificationsService', function($http, ApiUri, WarningsService) {
        ...
        function addNotification(entityName, assign, id, IsWatched, DropDownIsWatched, time) {
            notifications.push({
                //entity: 'Task',
                entity: 'task',
        ...
        function addLastnotifications() {
            ...
            if (notifications.length != 0) {
                lastNotification1.push({
                    ...
                    date: notifications[notifications.length - 1].date,
                    id: notifications[notifications.length - 1].id,
                    time: notifications[notifications.length - 1].time
                });

                //lastNotification1.push(notifications[notifications.length - 1]);
            }
            console.log(lastNotification1)
        }
