...
angular.module('mean.icu.ui.entityDetails', []).controller('EntityDetailsController', function($scope, $rootScope, entity, tasks, people, projects, tags, $timeout, context, $state, ProjectsService, ActivitiesService, PermissionsService, EntityService, $stateParams, me) {
    $scope.me = me;
    ...
    //due start
    if ($scope.project.due)
        $scope.project.due = new Date($scope.project.due);
    ...
    $scope.getUnusedTags = function() {
        ...
        return $scope.tags.filter(function(x) {
            return $scope.project.tags.indexOf(x) < 0
        })
    }
    ;
    ...
    if ($scope.project && ($state.current.name === 'main.projects.all.details' || $state.current.name === 'main.search.project' || $state.current.name === 'main.projects.byentity.details')) {
        $state.go('.activities');
    }
});
