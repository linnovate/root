...
(function() {
  ...
  describe("MEAN controllers", function() {
    describe("ArticlesController", function() {
      ...
      it(
        "$scope.find() should create an array with at least one article object " +
          "fetched from XHR",
        function() {
      ...
      it(
        "$scope.findOne() should create an array with one article object fetched " +
        ...
        function() {
          ...
          $httpBackend
            .expectGET(/api\/articles\/([0-9a-fA-F]{24})$/)
      ...
      it(
        "$scope.create() with valid form data should send a POST request " +
        ...
        function() {
          ...
          $httpBackend
            .expectPOST("api/articles", postArticleData())
            .respond(responseArticleData());
          ...
          expect($location.path()).toBe(
            "/articles/" + responseArticleData()._id
          );
        }
      );

      it("$scope.update(true) should update a valid article", inject(function(
        Articles
      ) {
        // fixture rideshare
      ...
      it(
        "$scope.remove() should send a DELETE request with a valid articleId " +
          "and remove the article from the scope",
        inject(function(Articles) {
          ...
          $httpBackend
            .expectDELETE(/api\/articles\/([0-9a-fA-F]{24})$/)
            .respond(204);
