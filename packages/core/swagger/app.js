...
Swagger.register(function(app, auth, database) {
  ...
  var sortParm = paramTypes.query(
    "sort",
    'Comma seperated list of params to sort by.  (e.g "-created,name") ',
    "string"
  );
  var limitParm = paramTypes.query(
    "limit",
  ...
  defaultGetParams = [sortParm, limitParm, skipParm];
  ...
  Swagger.add = function(path) {
    ...
    require(path + "/docs/services").load(swagger, {
  ...
  swagger.setApiInfo({
    ...
    licenseUrl:
      config.swagger.license || "http://en.wikipedia.org/wiki/MIT_License"
  });
  ...
  app.get("/api/docs", function(req, res, next) {
    Swagger.render("index", { endpoint: "/api/swagger/docs" }, function(
      err,
      html
    ) {
