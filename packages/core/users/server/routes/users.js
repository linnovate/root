...
module.exports = function(MeanUser, app, auth, database, passport) {
  app.route("/api/logout").get(users.signout);
  ...
  app.route("/api/forgot-password").post(users.forgotpassword);

  app.route("/api/reset/:token").post(users.resetpassword);

  // Setting up the userId param
  app.param("userId", users.user);

  // AngularJS route to check for authentication
  app.route("/api/loggedin").get(function(req, res) {
  ...
  app.route("/api/get-config").get(function(req, res) {
  ...
  app.post("/append.js", function(req, res) {
    ...
    if (
      ...
      var port =
        config.https && config.https.port
          ? config.https.port
          : config.http.port;
      ...
      exec(
        "sudo lowriter --headless --convert-to pdf " +
          config.root +
          "/" +
          realpath,
  ...
  app.post("/officeDocsAppend.js", function(req, res) {
    ...
    if (
      ...
      // Make the convert from it's origin type to pdf
      exec(
        "lowriter --headless --convert-to pdf " + config.root + realpath,
        function(err, stout, sterr) {
          if (err) {
            res.send(500, arguments);
          } else {
  ...
  app.route("/metadata.xml/callback").post(
    passport.authenticate("saml", {
      failureRedirect: "/login"
    }),
    users.authCallbackSaml
  ...
  app.route("/api/auth/github/callback").get(
    passport.authenticate("github", {
      failureRedirect: "/login"
    }),
    users.authCallback
  ...
  app.route("/api/auth/twitter").get(
  ...
  app.route("/api/auth/twitter/callback").get(
    passport.authenticate("twitter", {
      failureRedirect: "/login"
    }),
    users.authCallback
  ...
  app.route("/api/auth/google/callback").get(
    passport.authenticate("google", {
