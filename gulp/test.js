...
gulp.task("webdriver-update", function(done) {
  child_process
    .spawn(
      require.resolve("protractor/bin/webdriver-manager"),
      ["update", "--standalone"],
      {
        stdio: "inherit"
      }
...
gulp.task("e2e", function(done) {
  child_process
    .spawn(require.resolve("protractor/bin/protractor"), ["test/e2e/config"], {
      stdio: "inherit"
    })
    .once("close", done);
});
...
gulp.task("test", function(done) {
  runSequence("webdriver-update", "test-server", "e2e", function() {
    server && server.kill();
    done();
  });
});
